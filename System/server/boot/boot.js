...
var _boot = (function($tw) {
	...
	$tw.utils.error = function(err) {
		...
		if ($tw.browser && !$tw.node) {
			...
			var dm = $tw.utils.domMaker,
				...
				button = dm("button", {
					text: ($tw.language == undefined ? "close" : $tw.language.getString("Buttons/Close/Caption"))
				}),
				form = dm("form", {
		...
		if (!$tw.browser) {
			...
			*/
			Object.defineProperty(sandbox.require, "main", {
				value: (typeof(require) !== "undefined") ? require.main : {
					TiddlyWiki: _boot
				},
				writable: false,
				enumerable: true,
	...
	*/
	$tw.Wiki.prototype.processSafeMode = function() {
		...
		overrides.forEach(function(title) {
			...
			self.addTiddler(new $tw.Tiddler(tiddler, {
				title: newTitle
			}));
			report.push("* [[" + title + "|" + newTitle + "]]");
		});
		report.push()
		this.addTiddler(new $tw.Tiddler({
			title: titleReportTiddler,
			text: report.join("\n\n")
		}));
	...
	*/
	...
	if ($tw.node) {
		/*
		Simply return an array of all files in the folder recursively
		*/
		...
		var recursiveLoadFromPath = function(dir, filelist) {
			...
			files.forEach(function(file) {
				if (fs.statSync(path.join(dir, file)).isDirectory()) {
					filelist = recursiveLoadFromPath(path.join(dir, file), filelist);
				} else {
			...
			$tw.utils.each(filesInfo.directories, function(dirSpec) {
				...
				if (typeof dirSpec === "string") {
					...
					var dirPath = path.resolve(filepath, dirSpec.path),
						files = recursiveLoadFromPath(dirPath),
		...
		*/
		$tw.loadWikiTiddlers = function(wikiPath, options) {
			...
			var resolvedWikiPathmain = path.resolve(wikiPath, wikiInfo.wikiTiddlersDir);
			$tw.utils.each($tw.loadTiddlersFromPath(resolvedWikiPathmain), function(tiddlerFile) {
			...
			wikiInfo.includeDir.forEach(function(directory) {
			...
			$tw.boot.wikiTiddlersPath = path.resolve($tw.boot.wikiPath, config["default-tiddler-location"] || wikiInfo.wikiTiddlersDir);
	...
	$tw.boot.startup = function(options) {
		...
		$tw.utils.deepDefaults($tw, {
			...
			config: { // Configuration overridables
				...
				wikiTiddlersSubDir: ["./functions", "./templates", "./contents"],
